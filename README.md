...
<img src="assets/banner.png" alt="RiskGuard — 给你的交易系统配一个永不情绪化的风控官" width="100%">
...
风控的本质不是技术,而是**提前写死的纪律**。RiskGuard 把那几条能挡住绝大多数爆仓惨剧的规则,做成一个核心零依赖、可组合的库,让系统自动执行——因为等你真的亏钱上头那一刻,意志力是这世界上最靠不住的东西。

<details>
<summary><strong>📖 目录</strong></summary>

- [看它拦下一次爆仓(30 秒)](#看它拦下一次爆仓30-秒)
- [为什么不自己写几个 if 判断就好?](#为什么不自己写几个-if-判断就好)
- [一套风控,这些能力](#一套风控这些能力)
- [60 秒上手](#60-秒上手)
- [动态下注](#动态下注让公式算不让情绪算)
- [实时哨兵(kill-switch)](#实时哨兵kill-switch)
- [防篡改审计](#防篡改审计)
- [状态持久化](#状态持久化堵住重启绕过熔断的后门)
- [每日体检 + 压力测试](#每日体检-压力测试ai-帮你盯盘的那一层)
- [接进你的回测框架](#接进你的回测框架)
- [三档预设 + 命令行](#三档预设-命令行)
- [架构](#架构)
- [适合谁 / 设计原则 / 边界 / 路线图](#适合谁)
...
| ⏱️ 日内亏损线 | `DailyLossLimit` | 当日相对开盘锚定亏 `max_daily_loss_pct`(预设 2–5%)即停新仓,换日自动复位——今天到此为止就是到此为止 |
| 🧤 价格保护带 | `PriceBandRule` | 限价偏离市价超 `max_price_band_pct` 即拒——AI 把 200 打成 2000 的 fat-finger 单走不到券商 |
| 🚥 下单节流 | `OrderThrottle` | 滚动窗口订单数触顶即拒——失控的 AI 循环最多烧掉一个窗口的配额 |
| 🚨 实时哨兵 | `RiskMonitor` | 后台线程盯盘,触线自动踩刹车(撤单 + 平仓)——情绪失控时替你拉闸 |
| 🌙 每日体检 / 压力测试 | `build_digest` / `run_stress_test` | 结构化摘要给 AI agent 叙述;"如果集体跌 20%"的一次性推演,零副作用 |
| 📿 防篡改案底 | `JsonlAuditSink` / `SqliteAuditSink` | 每次裁决/熔断/成交都留哈希链记录,可选 HMAC 防伪 |
| 🔒 状态持久化 | `SqliteStateStore` | 高水位/熔断状态写透磁盘,重启不再是绕过风控的后门 |
| 🔌 可插拔执行层 | `Broker` / `PaperBroker` / `AlpacaBroker` | 纸面模拟盘、Alpaca、或你自研后端,实现一个接口即可接入 |
...
## 🤖 AI 代理闸门:让 AI 动脑,别让它碰扳机

"AI 可以研究、写码、挑毛病,但绝不能直接下单"——说起来容易,做起来有个漏洞:

**AI 特有的失效模式,老三条铁律拦不住。** 失控的代理循环可以在一分钟里打出
几百笔"单笔都合规"的订单;把 200 敲成 2000 的 fat-finger 限价单,仓位上限照样放行;
当日从平盘急跌 3%,离 15% 的历史高点回撤线还远得很。三件套逐个补上:

```python
from riskguard import RiskEngine, get_preset

engine = RiskEngine(get_preset("balanced"))
# balanced 档自动带上:日内亏损线 3% / 价格带 ±10% / 节流 10 单每分钟、120 单每小时
# 日内线触发后当日粘性(回血也不解锁,防赌徒循环),换日自动复位;
# 减仓单默认完全豁免节流("减仓永远放行"原则原样保留)。想让失控的减仓循环
# 也有限度?显式设 reduce_only_throttle_factor(如 5.0)opt-in 一个放宽的上限。
```

## 🚨 实时哨兵(kill-switch)

光把纪律写进配置还不够——你睡觉、开会、没信号的时候,市场不会为了配合你的作息暂停。`RiskMonitor` 是一条永不闭眼的后台线程,周期性盯着账户,触线自动踩刹车:
...
## 🌙 每日体检 + 压力测试:AI 帮你盯盘的那一层

"AI 帮你 24 小时盯盘"这个刚需,拆开看是四层:实时监控、异常检测、硬编码熔断线、压力测试。**硬编码熔断线**是 RiskGuard 从第一行代码起就在做的事(上面几节都是);这里补上另外两层里**属于本库使命范围**的部分——**每日体检**(把"实时监控"摊开成一份可复算的摘要)和**压力测试**(把"如果……会怎样"摊开成一份推演)。
...
CLI 同样支持,适合脚本化的"每天 check-in"和"随手问一句"([`examples/11`](examples/11_daily_digest.py)、[`examples/12`](examples/12_stress_test.py)):
...
> **压力测试的只读承诺**:不触发熔断、不写审计、不碰持久化——即便 `--state-db` 指向一个还不存在的文件,也不会在磁盘上留下任何新文件。它是纯粹的"假设推演",算完就完。
...
```
     ...
     Kelly·波动率·固定  │          都是同一个 RiskEngine 的不同调用方,只读或推演
        ...
        └───────────────┬──────────────────┘
                        │ 放行的订单(线程安全、加锁)
                        ▼
                  Broker 抽象层 ── PaperBroker / AlpacaBroker / 自研

     📿 审计(JSONL/SQLite,可选 HMAC) · 🚨 RiskMonitor(kill-switch,直连 Broker)
```

## 🙋 适合谁

- 已经在用 backtesting.py / vectorbt / 自研执行层,缺一个**不绑定任何框架**的风控中间件的
- 想让 AI agent 帮你研究、下单,但**不敢让它直接碰真实资金**的——RiskGuard 就是那道 AI 绕不过去的闸门
- 爆过一次仓、亏过一次意气之战,想把"纪律"从脑子里搬进代码、不再靠意志力硬扛的
- 需要一个能接 Alpaca / 自研后端的风控中间层的
...
- **本库不做异常检测/行情判断**——没有新闻源、没有情绪分析、没有"这次不一样"式的模糊判断。它只做一件事:把你自己定好的硬规则,毫不犹豫地执行。
...
- [x] 每日体检 + 压力测试
- [x] 状态持久化:堵住"重启绕过熔断"的后门
- [x] CI(pytest 矩阵 + ruff)
- [x] 配置预设 + CLI:保守/均衡/激进三档 + 命令行
- [x] 与 backtesting.py / vectorbt 的一键接线
- [ ] 审计外部锚定(WORM / 公证)
- [x] AI 代理闸门三件套:日内亏损线、fat-finger 价格保护带、下单频率节流
- [ ] CLI 输出国际化(目前命令行打印内容仅支持中文;库本身的输出已是英文)

已完成的版本历史见 [CHANGELOG](CHANGELOG.md)。

## 🛠️ 开发

```bash
pip install -e ".[dev]"
pytest            # 866 passed,CI 跑 3.10–3.13
ruff check src     # 干净
